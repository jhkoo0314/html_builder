...
function isTextLike(filePath) {
  return [".js", ".cjs", ".mjs", ".ts", ".tsx", ".jsx"].includes(path.extname(filePath).toLowerCase());
}
...
function scanCrossImports(baseDir, forbiddenTokens) {
  ...
  const files = walkFiles(srcDir).filter(isTextLike);
  const importRegexes = forbiddenTokens.map(
    (token) =>
      new RegExp(
        String.raw`(require\(|from\s+|import\s+.*from\s+|["'])[^"'\\n]*${token.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}[^"'\\n]*["']`
      )
  );
  for (const filePath of files) {
    const rel = path.relative(repoRoot, filePath);
    const text = safeReadText(filePath);
    for (let i = 0; i < forbiddenTokens.length; i += 1) {
      if (importRegexes[i].test(text)) {
        violations.push(`${rel} -> token "${forbiddenTokens[i]}"`);
      }
    }
  }
  return violations;
